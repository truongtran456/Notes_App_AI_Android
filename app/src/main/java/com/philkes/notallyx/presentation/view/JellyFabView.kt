...
/**
 * JellyFabView - A jelly-like expandable FAB menu inspired by JellyFab Supports two-layer
 * expansion: primary and secondary
 */
class JellyFabView
@JvmOverloads
constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) :
    ...
    init {
        clipChildren = false
        clipToPadding = false
        // ??m b?o view c� background trong su?t ?? kh�ng che c�c view kh�c
    ...
    fun setup(
        ...
        mainFab =
            FloatingActionButton(context, null, R.style.FloatingActionButtonPrimary).apply {
                ...
                // Set màu tím đậm giống FAB ở trang Notes - dùng style FloatingActionButtonPrimary
                // Style này đã có bg_fab_gradient với màu #6D3FEF
                // Nhưng để đảm bảo, set lại backgroundTintList sau khi apply style
        ...
        val mainFabParams =
            LayoutParams(fabSize, fabSize).apply {
                ...
                // ??t v? tr� ban ??u ? g�c d??i b�n ph?i
                setMargins(0, 0, 0, 0)
            }
        addView(mainFab, mainFabParams)

        // ??m b?o mainFab ???c ??t ?�ng v? tr� ngay l?p t?c
        post {
            if (width > 0 && height > 0) {
        ...
        primaryItems.forEachIndexed { index, item ->
            val fab =
                FloatingActionButton(context, null, R.style.FloatingActionButtonPrimary).apply {
                    ...
                    scaleY = 0f
                    visibility = GONE
                    // Style FloatingActionButtonPrimary đã có bg_fab_gradient với màu #6D3FEF
                    setOnClickListener {
        ...
        // Request layout ?? ??m b?o c� k�ch th??c tr??c khi t�nh to�n
        post {
            calculatePositions()
            updateFabPositions()
        }
    ...
    private fun calculatePositions() {
        ...
        // ??m b?o c� k�ch th??c tr??c khi t�nh to�n
        if (width == 0 || height == 0) {
            return
        }
        ...
        if (moreFabIndex >= 0 && moreFabIndex < primaryFabPositions.size) {
            ...
            if (secondaryCount > 0) {
                ...
                secondaryFabs.forEachIndexed { index, _ ->
                    val angle =
                        Math.toRadians(
                            (arcStartAngle + secondaryAngleStep * (index + 1)).toDouble()
                        )
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        // ??m b?o c� k�ch th??c t?i thi?u ?? ch?a FABs
    ...
    private fun updateFabPositions() {
        ...
        // Main FAB - ??t ? g�c d??i b�n ph?i
        if (::mainFab.isInitialized) {
            mainFab.x = centerX - fabSize / 2f
            mainFab.y = centerY - fabSize / 2f
        }

        // Primary FABs - ch? update n?u ?� c� positions
        ...
        // Secondary FABs - ch? update n?u ?� c� positions
        ...
        scrimView?.let {
            ...
            scrimAnimator =
                ValueAnimator.ofFloat(0f, 0.3f).apply {
                    duration = 300
                    addUpdateListener { animator -> it.alpha = animator.animatedValue as Float }
                    start()
                }
        }
    ...
    private fun collapseSecondary() {
        ...
        secondaryFabs.forEachIndexed { index, fab ->
            fab.animate()
                ...
                .setDuration(200)
                .withEndAction { fab.visibility = GONE }
                .start()
        }
    }
    ...
    private fun collapseAllInternal() {
        ...
        scrimView?.let {
            scrimAnimator?.cancel()
            scrimAnimator =
                ValueAnimator.ofFloat(it.alpha, 0f).apply {
                    duration = 200
                    addUpdateListener { animator -> it.alpha = animator.animatedValue as Float }
                    addListener(
