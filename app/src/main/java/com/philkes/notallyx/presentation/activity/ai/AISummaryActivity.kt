...
class AISummaryActivity : AppCompatActivity() {
    ...
    private var matchPairsScore: Int = 0 // S? t? ?� ho�n th�nh (?� match ?�ng �t nh?t 1 l?n)
    ...
    private fun setupToolbar() {
        ...
        // Set title d?a tr�n initialSection
        val title =
            when (initialSection) {
                // Text-mode sections
    ...
    private fun summarizeNote() {
        ...
        lifecycleScope.launch {
            try {
                // ??m b?o note_id g?i l�n backend lu�n l� UUID duy nh?t (tr�nh tr�ng sau khi wipe
                // app)
                ...
                // N?u n�n d�ng cache v� ?� c� noteId, th? GET tr??c ?? tr�nh g?i POST
                ...
                // Lu�n d�ng /process/combined cho note text (text + future files) ?? ??ng b? cache
                val result =
    ...
    /**
     * ??m b?o note_id g?i l�n backend lu�n l� UUID duy nh?t ?? tr�nh tr�ng v?i c�c l?n c�i app
     * tr??c ?� (khi local noteId c� th? l?p l?i sau khi wipe).
     */
    private fun ensureBackendNoteId(): String {
        ...
        // N?u c� local noteId, l?u mapping ?? l?n sau v?n d�ng c�ng backendNoteId (khi hash kh?p)
        if (noteId != -1L) {
    ...
    private fun displayResults(
        ...
        if (!isVocabMode) {
            if (initialSection == AISection.MCQ) {
                // Text MCQ: chuyển sang flow quiz mới (full-screen activity)
                binding.MCQCard.isVisible = false
                startTextMcqFlow(response.mcqs)
                // Ẩn các card khác để tập trung vào quiz
        ...
        // Stats-only mode: ?n c�c n?i dung kh�c, ch? hi?n th? ti?n ?? v� b?ng th?ng k�
        if (statsOnly) {
        ...
        // LU�N set summaryTableState ?? Match Pairs c� th? l?y ngh?a ti?ng Vi?t
        // (ngay c? khi kh�ng hi?n th? Summary Table card)
        ...
        // C?p nh?t Overall Progress Card (n?u c� d? li?u quiz)
    ...
    private fun updateVocabStoryCard(story: VocabStory?) {
        ...
        if (paragraphs.isEmpty()) {
            // N?u kh�ng c� paragraphs, hi?n th? th�ng b�o
    ...
    private fun updateVocabMCQCard(quizzes: List<VocabQuiz>?) {
        ...
        if (noteId != -1L && currentSetId != null) {
            ...
            if (savedSetId != -1 && savedSetId != currentSetId) {
                // set_id thay ??i -> reset ti?n tr�nh MCQ
                android.util.Log.d(
                    "AISummaryActivity",
        ...
        if (isCompleted) {
            ...
            // Kh�i ph?c th? t? c�u h?i v� ?�p �n ?� ch?n t? prefs
            val gson = Gson()
            ...
            // S?p x?p l?i theo order ?� l?u n?u c�, ?? map ?�p �n v? ?�ng c�u
            val orderedItems =
            ...
            // Kh�i ph?c ?�p �n ng??i d�ng theo id
            vocabMCQUserAnswers.clear()
        ...
        // Checklist mode: hi?n th? t?ng c�u h?i ng?u nhi�n, t�nh ?i?m
    ...
    private fun showVocabMCQResult() {
        ...
        // Load k?t qu? ?� l?u n?u c� (khi m? l?i)
        if (noteId != -1L) {
            val prefs = getSharedPreferences("quiz_results", Context.MODE_PRIVATE)
    ...
    private fun createVocabMCQView(quiz: VocabQuiz): View {
        ...
        // Question text (kh�ng hi?n th? Target ?? kh�ng l? ?�p �n)
        val questionText =
            TextView(this).apply {
                val questionStr = quiz.question ?: ""
                text = markdownBoldToSpannable(questionStr)
    ...
    private fun updateClozeCard(clozeTests: List<ClozeTest>?) {
        ...
        // L?y set_id t? cloze_tests ??u ti�n (t?t c? ??u c� c�ng set_id)
        val currentSetId = clozeTests.firstOrNull()?.setId

        // Check xem set_id c� thay ??i kh�ng (n?u c� th� reset progress)
        if (noteId != -1L) {
            val prefs = getSharedPreferences("quiz_results", Context.MODE_PRIVATE)
        ...
        if (isCompleted) {
            ...
            if (noteId != -1L) {
                ...
                // Kh�i ph?c th? t? c�u h?i v� ?�p �n ?� ch?n
                val gson = Gson()
                ...
                // Kh�i ph?c c�u tr? l?i ng??i d�ng theo id
                clozeUserAnswersQuiz.clear()
                if (!answersById.isNullOrEmpty()) {
        ...
        // Checklist mode: hi?n th? t?ng c�u h?i ng?u nhi�n, t�nh ?i?m
        // T�ch m?i blank th�nh m?t c�u h?i ri�ng
        clozeAllQuestions.clear()
    ...
    private fun showOverallStatistics(container: LinearLayout) {
        // N?u ?ang ? ch? ?? statsOnly: t�nh to�n tr?c ti?p t? SummaryResponse + SharedPreferences
    ...
    private fun updateOverallProgressCardSummary() {
        // N?u ch?a c� d? li?u vocab/quizzes th� ?n card
        if (
    ...
    private fun updateMatchPairsCard(pairs: List<MatchPair>?) {
        ...
        // L?y set_id t? match_pairs ??u ti�n (t?t c? ??u c� c�ng set_id)
        ...
        // L?y t?t c? c�c t? unique ?? t�nh ?i?m theo t?
        ...
        // Load progress ?� l?u (n?u c�)
        matchPairsWordsMatched.clear()
        matchPairsVocabProgress.clear()
        loadMatchPairsVocabProgress()

        // ??m b?o t?t c? vocab ??u c� entry trong progress
        matchPairsUniqueWords.forEach { vocab ->
        ...
        fun shortenMeaning(meaning: String?): String {
            ...
            // L?y ph?n ??u ti�n, lo?i b? d?u c�u
            val firstPart = cleaned.split(Regex("[,?.?()??\\-��]")).firstOrNull()?.trim() ?: cleaned
            // Gi?i h?n 25 k� t? ?? v?a v?i �
            return if (firstPart.length > 25) firstPart.take(25).trim() + "..." else firstPart
        }

        /**
         * Render m?t l??t (round) 4x4 v?i t?i ?a 8 t? v?ng, ?u ti�n c�c t? CH?A completed, ch?m
         * ?i?m theo t?.
         *
         * Sau khi ho�n th�nh h?t c�c t? trong round hi?n t?i:
         * - N?u v?n c�n t? ch?a ho�n th�nh trong note ? t? ??ng render round m?i.
        ...
        fun renderRound() {
            ...
            // N?u ?� ho�n th�nh h?t th� hi?n th? k?t qu?
            if (
            ...
            // ?u ti�n vocab c� attempts cao h?n (b? sai nhi?u) r?i shuffle v� l?y t?i ?a 8
            ...
            // Ch?n 1 pair cho m?i vocab (n?u c� nhi?u pair cho c�ng 1 t? th� l?y c?p ??u ti�n)
            ...
            // T�nh s? c?t v� h�ng d?a tr�n s? l??ng tiles (4x4 ho?c �t h?n nh?ng v?n 4 c?t)
            val totalTiles = tiles.size
            val columnCount = 4
            val rowCount = (totalTiles + columnCount - 1) / columnCount // L�m tr�n l�n
                        ...
                        setOnClickListener {
                            ...
                            if (first == null) {
                                ...
                                if (first == secondKey) {
                                    ...
                                    // N?u t? n�y ch?a ???c match ?�ng tr??c ?�, ?�nh d?u completed
                                    // + t?ng ?i?m
                                    if (!matchPairsWordsMatched.contains(vocabKey)) {
                                        matchPairsWordsMatched.add(vocabKey)
                                        matchPairsScore =
                                            matchPairsWordsMatched
                                                .size // ?i?m t�nh theo s? vocab ?� ho�n th�nh

                                        if (prog != null) {
                                            prog.status = "completed"
                                            prog.completedAt = System.currentTimeMillis()
                                            // C?ng th�m 1 attempt cho l?n match ?�ng n�y
                                    ...
                                    if (matchPairsWordsMatched.size >= matchPairsUniqueWords.size) {
                                        ...
                                        // currentRoundWords),
                                        // nh?ng v?n c�n t? ch?a ho�n th�nh trong note ? render
                                        // round m?i.
                                        val roundCompleted =
                                    ...
                                    // Match sai -> t?ng attempts cho vocab ?�
                                    val vocabKey = tileInfo.wordKey.lowercase().trim()
                                    val prog = matchPairsVocabProgress[vocabKey]
                                    if (prog != null) {
                                        prog.attempts = (prog.attempts + 1).coerceAtMost(999)
            ...
            // B? n�t reset v� ?� hi?n th? t?t c? c�c c?p, kh�ng c?n reset n?a
    ...
    /**
     * ?n to�n b? card n?i dung, ch? gi? Overall Progress v� b?ng th?ng k� per vocab. D�ng cho ch?
     * ?? statsOnly.
     */
    ...
    /**
     * Render b?ng th?ng k� per-vocab v� % t?ng, ch? d�ng cho statsOnly. T?n d?ng h�m
     * showOverallStatistics ?? t�nh to�n l?i.
     */
    private fun renderStatsOnly() {
        ...
        // T?o container m?i cho b?ng th?ng k�
        val statsContainer =
            LinearLayout(this).apply {
                orientation = LinearLayout.VERTICAL
        ...
        // Th�m v�o cu?i n?i dung ScrollView
    ...
    private fun updateSummaryTableCard(rows: List<VocabSummaryRow>?) {
        ...
        items.forEachIndexed { index, row ->
            ...
            if (word.isNotBlank()) {
                ...
                val speakBtn =
                    ImageButton(this).apply {
                        ...
                        setOnClickListener {
                            // Lu�n d�ng t? g?c (word) ?? TTS ??c, kh�ng d�ng phonetic v� TTS kh�ng
                            // hi?u IPA notation
            ...
            // Translation
            row.translation?.let { addField("Dịch", it) }

            // Part of Speech
            row.partOfSpeech?.let { addField("Loại từ", it) }

            // Definition
            row.definition?.let { addField("Định nghĩa", it) }
            ...
            row.commonStructures
                ...
                ?.let { addField("Cấu trúc", it.joinToString(", ")) }

            // Collocations
            row.collocations
                ?.takeIf { it.isNotEmpty() }
                ?.let { addField("Cụm từ", it.joinToString(", ")) }
    ...
    private fun translateVocabStory() {
        // ?u ti�n l?y t? bi?n instance (?� hi?n th?), n?u kh�ng c� th� l?y t? summaryResponse
        val story =
            vocabStoryOriginal
    ...
    private fun translateVocabMCQ() {
        ...
        val textToTranslate = buildString {
            quizzes.forEachIndexed { index, quiz ->
                ...
                // Kh�ng hi?n th? Target ?? kh�ng l? ?�p �n
